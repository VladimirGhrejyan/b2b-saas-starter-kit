...
| `general/discriminated-unions.mdc` | Union discriminants are enums, not string-literal types       |
...
| `general/declaration-files.mdc`    | Ambient `.d.ts` files always live in an `@types` folder       |
...
| `backend/audit-and-versioning.mdc` | Row audit timestamps, optimistic version, child-collection writes |
