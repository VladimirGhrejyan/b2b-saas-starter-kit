...
import type {TenantActor, TenantId, UserId} from '@b2b-saas-starter-kit/shared-kernel-types'
...
export class TenantsService {
  ...
  async get(tenantId: TenantId, actor: TenantActor): Promise<GetTenantOutputDto> {
    const result = await this.getTenant.execute(GetTenantMapper.toQuery(tenantId, actor))
