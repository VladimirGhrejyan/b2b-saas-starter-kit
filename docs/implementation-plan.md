...
| **Authentication**  | **Implemented (Phase 20).** Email + password (Argon2id), short JWT access, cookie refresh on `/v1/auth/web/*` and JSON `refreshToken` on `/v1/auth/{login,refresh,logout}`, set/change password, tenant claim after selection. SMTP when `SMTP_HOST` is set; otherwise `LoggingMailer`. Web login stores the access token in memory and sends `Authorization: Bearer`; a 401 retries once via the refresh cookie. API `development`/`test` still accepts `x-user-id` / `x-tenant-id` for e2e. Production is JWT-only and refuses the default secret. |
...
| **Redis**           | **Implemented (Phase 18 + 21).** `CachePort` / `LockPort` / `PubSubPort` / `RateLimiterPort` on `platform`; adapters in `packages/infrastructure/redis`. Effective permissions are cache-aside (tenant-prefixed key, 60s TTL) behind `AuthorizationPort`, with `invalidate` / `invalidateHoldersOf` on membership and role writes. Public `/v1/auth/*` is fixed-window rate-limited. BullMQ and outbox remain deferred.                                                                                                                              |
...
Out of scope for **Part 1 (backend)**: Redis extras beyond Phase 18, transactional outbox + domain-event bus, `audit` and `notifications` contexts, `apps/worker` wiring, Postgres RLS, `gateway`/realtime. Frontend is **Part 2**, not deferred as a blob. Web login + Bearer `prepareHeaders` are shipped after Phase 20.
...
- **Deferred at the time:** `CachePort`/`LockPort`/`PubSubPort` (landed in Phase 18). `Logger` port + locator is Phase 9 (platform package already exists).
...
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Real authentication**                | **Done (Phase 20).** Native token paths, cookie `/auth/web/*`, set/change password, SMTP when `SMTP_HOST` is set, web `/login`, in-memory access token, Bearer `prepareHeaders`, `credentials: 'include'`, and 401 → cookie refresh. Logout UI, select-tenant UI, register/forgot/reset pages, SSO, passkeys, email verification, MFA, outbox, and HTML templates are still deferred. |
| **Redis**                              | **Done (Phase 18 + 21).** Permission-cache `del` and public auth rate limiting are implemented. BullMQ / `messaging`, invite limits, global API throttling, and transactional outbox remain deferred.                                                                                                                                                                                 |
...
| **Custom roles + invitations**         | **Done.** Email invite + accept, attach-existing, replace membership roles, and custom-role CRUD are on `/v1`. SMTP is used when `SMTP_HOST` is set. Outbox/`MemberInvited`, HTML templates, suspend, and Owner transfer remain deferred.                                                                                                                                             |
...
- **Deferred:** create-user / create-tenant forms, invitations, logout / select-tenant / register-forgot-reset UI, tenant branding from API (waits on UI tech), copying members into `apps/admin` or a `frontend/feature-*` lib.
...
### Phase 18 — Redis ports + capability adapters

- **Goal:** platform Cache/Lock/PubSub ports and a capability-folder `packages/infrastructure/redis` (ioredis), with effective permissions cache-aside behind the existing `AuthorizationPort`.
- **Scope:** `CachePort` (`ttlSeconds` required on `set`), `LockPort`, `PubSubPort`, `CacheKey.tenant` on `platform`; redis kernel (config, shared ioredis client, Nest `forRootAsync`) plus `cache/` / `lock/` / `pubsub/` adapters. Vertical slice: `AuthorizationService.getEffectivePermissions` is cache-aside (tenant-prefixed key, 60s TTL). Lock/PubSub ship and are integration-tested but are not wired into use cases yet.
- **Packages/projects:** create `packages/infrastructure/redis` (Nx name `redis`, tags `scope:backend`, `layer:infrastructure` — **no** extra `layer:redis`). Depends on `platform` + `config` (+ `ioredis`), not `domain` / `application`. `apps/api` must **not** import `@b2b-saas-starter-kit/redis`.
- **Implementation tasks:**
  - Scaffold with `@nx/js:library`; ESLint carve-out allows `ioredis` only under `packages/infrastructure/redis/**` (postgres/logger still ban it).
  - In-memory `CachePort` fake in `application/testing`; composition imports `RedisInfrastructureModule.forRootAsync` and injects `CACHE` into `AuthorizationService`.
  - Document that future role/membership writes must `del` the same key (no write-path invalidation yet).
- **Tests:** platform key-builder; application cache hit skips repo work and tenant A does not leak into tenant B; redis integration against compose (logical DB `1`, `FLUSHDB` that DB only); existing HTTP e2e still green with Redis running.
...
- **Deferred:** BullMQ / `messaging`, transactional outbox, pub/sub invalidation of permission keys, `apps/worker`, Redis as session store. `RateLimiterPort` is Phase 21.
...
- **Goal:** Real email+password authentication on the API: Argon2id local credentials, short-lived JWT access, cookie vs native refresh transports, tenant claim after selection, set/change password, password-reset via `MailerPort`. Web stays on `x-user-id` / `x-tenant-id` until a later frontend phase.
- **Scope:** `User` stays credential-free. JWT sign/verify stays in `apps/api` (`jose`). Mailer is `SmtpMailer` when `SMTP_HOST` is set, otherwise `LoggingMailer` / `InMemoryMailer` in tests. Production boots only with a non-default `JWT_ACCESS_SECRET` and does not trust headers.
...
- **Deferred:** logout UI, select-tenant UI, register/forgot/reset pages, outbox, HTML templates, SSO, passkeys, MFA. Web login + Bearer `prepareHeaders` + 401 refresh are shipped.

### Phase 21 — Auth rate limiter

- **Goal:** Redis fixed-window `RateLimiterPort` on public `/v1/auth/*` (login, register, refresh, logout, forgot, reset). No account lockout. `select-tenant` and `POST /auth/password` stay unlimited.
- **Scope:** platform port + `RateLimitExceededError`; `RedisRateLimiter` Lua `INCR`/`EXPIRE`; `@RateLimit` + interceptor in `apps/api`; `429` + `Retry-After`. Fail-open if Redis throws.
- **Verification:** `pnpm nx run-many -t lint,typecheck,test -p platform,application,redis,composition,nest-http,contracts,api`.
- **Deferred:** invite limits, global API throttling, account lockout, `X-Forwarded-For` / trust proxy.
...
| Area                                             | Why deferred / how it slots in later                                                                                                                                                               |
| ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Real authentication UI**                       | **Done for the Phase 20 web slice** (`/login`, in-memory access token, Bearer `prepareHeaders`, 401 refresh, picker removed). Remaining: logout UI, select-tenant UI, register/forgot/reset pages. |
