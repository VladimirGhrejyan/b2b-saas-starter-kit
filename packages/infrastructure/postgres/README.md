...
`packages/infrastructure/` is a grouping directory (like `packages/shared/`). Each concern is its own Nx project: `postgres`, `logger`, `telemetry`, `redis`, `http-client`, `security`, `node`; `messaging` later.
